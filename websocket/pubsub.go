...
type PubSub struct {
	...
	connections map[string]structures.Set[*connHandler]
}
...
func (ps *PubSub) listen() {
	for {
		...
		case conn := <-ps.register:
			if conns, ok := ps.connections[conn.channel]; ok {
				conns.Add(conn)
			} else {
				ps.connections[conn.channel] = structures.NewSet(conn)
			}
		...
		case msg := <-ps.broadcast:
			if conns, ok := ps.connections[msg.channel]; ok {
...
func NewPubSub() *PubSub {
	ps := PubSub{
		...
		connections: make(map[string]structures.Set[*connHandler]),
	}
	go ps.listen()
	return &ps
}
